...
class TextWorldSimple(Environment):
    def __init__(self, task_spec: JSONObject, secrets: dict[str, str] = {}) -> None:
        ...
        settings = {
            "rewards": self.validated.rewards,
            "goal": self.validated.goal,
            "test": self.validated.split == "test",
        }

        options = GameOptions()
        options.seeds = self.validated.seed
        self._tmpdir = tempfile.mkdtemp()
        options.path = os.path.join(self._tmpdir, f"tw_simple_{self.validated.id}")

        game = make_simple_game(settings, options)
        self._walkthrough = game.metadata.get("walkthrough", [])
        self._game_objective = game.objective or ""
...
{initial_obs}"""
    ...
    async def send_command(self, params: SendCommandParams) -> ToolOutput:
        ...
        self._step_count += 1
        game_state, _cumulative_score, done = self._env.step(params.command)
